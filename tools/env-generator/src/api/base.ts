...
export class BaseCompiledTemplate {
    ...
    mergeWithDefaults(): void {
        this.environmentVariables = {
            ...
            'SESSION_COOKIE_PASSWORD': this.generateRandomString()
        }
    }
    ...
    validateEnv(prefix: string): EnvValidationError[] {
        ...
        const requiredVariables: string[] = ["rucio_host", "rucio_auth_host", "host", "project_url"]
        ...
        // check if multi vo is enabled, all vo variables are set
        if (env['MULTI_VO_ENABLED'] === 'true') {
          // check if vo_list is defined and a default VO is set
          if (!env['VO_LIST'] || !env['VO_DEFAULT']) {
            if(!env['VO_LIST']) {
                errors.push({
                  type: 'error',
                  message: `Multi VO is enabled. Please set a value for ${prefix}VO_LIST as csv string of vo short names`
              })
            }
            if(!env['VO_DEFAULT']) {
                errors.push({
                  type: 'error',
                  message: `Please set a value for ${prefix}VO_DEFAULT as the default vo's short name`
              })
            }
          } else{
            const voList = env['VO_LIST'].split(',')
            voList.forEach((vo) => {
    ...
    execute(): TTemplateGeneratorOutput {
        try {
            ...
            const content = this.engine.renderFileSync(this.templateName, this.environmentVariables)
            if(this.write) {
