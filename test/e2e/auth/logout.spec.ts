...
test.describe('Logout Functionality', () => {
    ...
    test('should clear all authentication cookies on logout', async ({ page }) => {
        ...
        const sessionCookieBefore = cookiesBeforeLogout.find(c => c.name.includes('session') || c.name.includes('rucio'));
        ...
        const sessionCookieAfter = cookiesAfterLogout.find(c => c.name === sessionCookieBefore?.name);

        // Verify session cookie is cleared or expired
        // It may still exist but should be expired or empty
        if (sessionCookieAfter) {
            expect(sessionCookieAfter.value === '' || sessionCookieAfter.expires < Date.now() / 1000).toBeTruthy();
        }
    });
});
