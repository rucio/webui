...
test.describe('Rule List Page', () => {
  ...
  test('should display rule status indicators', async ({ page }) => {
    ...
    const hasStatus = await statusTags.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should display account information for rules', async ({ page }) => {
    ...
    const hasAccount = await accountInfo.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should display RSE information for rules', async ({ page }) => {
    ...
    const hasRSE = await rseInfo.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should display create rule button or link', async ({ page }) => {
    ...
    const createButton = page.locator('button:has-text("Create"), a:has-text("Create"), button:has-text("New Rule"), a:has-text("New Rule")').first();
  ...
  test('should display DID information for rules', async ({ page }) => {
    ...
    const hasDID = await didInfo.first().isVisible({ timeout: 2000 }).catch(() => false);
