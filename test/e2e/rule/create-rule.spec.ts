...
test.describe('Create Rule Page', () => {
  ...
  test('should have DID input field', async ({ page }) => {
    ...
    const hasDIDInput = await didInputs.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should have RSE selection input', async ({ page }) => {
    ...
    const hasRSEInput = await rseInputs.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should have copies/replicas field', async ({ page }) => {
    ...
    const hasCopiesInput = await copiesInput.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should have lifetime field', async ({ page }) => {
    ...
    const hasLifetimeInput = await lifetimeInput.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should validate required fields', async ({ page }) => {
    ...
    const hasError = await page.locator('[role="alert"], .error, text=required, text=invalid').first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should display advanced options section', async ({ page }) => {
    ...
    const hasAdvanced = await advancedSection.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should support DID scope and name separate inputs', async ({ page }) => {
    ...
    const hasSeparateFields = await scopeInput.isVisible({ timeout: 2000 }).catch(() => false) &&
  ...
  test('should display help text or tooltips', async ({ page }) => {
    ...
    const hasHelp = await helpIcons.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should show RSE expression syntax help', async ({ page }) => {
    ...
    const hasRSEHelp = await rseHelp.first().isVisible({ timeout: 2000 }).catch(() => false);
