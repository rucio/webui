...
import { RuleViewModel } from '@/lib/core/view-model/rule';

@injectable()
export default class ListRulesUseCase
    extends BaseSingleEndpointStreamingUseCase<
        AuthenticatedRequestModel<ListRulesRequest>,
        ListRulesResponse,
        ListRulesError,
        ListRulesDTO,
        RuleDTO,
        RuleViewModel
    >
    implements ListRulesInputPort
{
    constructor(protected readonly presenter: ListRulesOutputPort, private readonly gateway: RuleGatewayOutputPort) {
        super(presenter);
    }
    ...
    async makeGatewayRequest(requestModel: AuthenticatedRequestModel<ListRulesRequest>): Promise<ListRulesDTO> {
        ...
        const dto: ListRulesDTO = await this.gateway.listRules(rucioAuthToken, filters);
        return dto;
    }
