...
async function reLogin(request: NextRequest, publicHost: string) {
    const logoutPage = new URL(
        `/api/auth/logout?callbackUrl=${request.nextUrl.pathname}`,
        `${publicHost}`,
    )
...
async function initiateLogin(request: NextRequest, publicHost: string) {
    const loginPage = new URL(
        `/api/auth/login?callbackUrl=${request.nextUrl.pathname}`,
        `${publicHost}`,
    )
...
export async function middleware(request: NextRequest) {
    ...
    try {
        ...
        console.log(error)
        return new Response(
            'Internal Server Error. Could not authenticate or rediect to login page',
            { status: 500 },
        )
    }
}
