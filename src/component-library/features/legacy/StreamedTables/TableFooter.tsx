    ...
    return (
        <tfoot className={twMerge('h-8', className ?? '')} role="rowgroup" aria-label="Table Footer" {...otherprops}>
            ...
            <tr aria-label="Pagination and Stream Controls">
                <td colSpan={props.table.getVisibleLeafColumns().length}>
                    ...
                    >
                        ...
                        />
                        <div className={twMerge('w-full md:w-[400px]')}>
                            <TablePaginationNav table={props.table} />
                        </div>
                        <TableFetchstatus
                            className={twMerge(
                                !props.stacked
                                    ? 'md:absolute md:inset-y-0 md:right-0 md:w-36 lg:w-48'
                                    : 'md:space-x-2 md:justify-center md:w-[400px]',
                                'w-full',
                            )}
                            comdom={props.comdom}
                        />
            ...
            <tr aria-label="View the error messages">
                <td colSpan={props.table.getVisibleLeafColumns().length} className={twMerge(showDetailedErrors ? 'table-cell' : 'hidden')}>
                    <TableErrorreader comdom={props.comdom} showDetailedErrors={showDetailedErrors} setShowDetailedErrors={setShowDetailedErrors} />
                </td>
            </tr>
        </tfoot>
    );
