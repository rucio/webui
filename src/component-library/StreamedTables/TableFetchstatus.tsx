...
const StateButton: (
    React.FC<JSX.IntrinsicElements["button"] & { icon: JSX.Element, state: FetchStatus }>
) = ({ icon, state, ...props }) => {
...
export const TableFetchstatus: (
    React.FC<JSX.IntrinsicElements["form"] & {
        ...
        return (
            ...
            >
                ...
                <div
                    className={twMerge(
                        "h-6",
                        state !== "idle" ? "hidden" : "inline-block"
                    )}
                >
                    <StateButton
