...
export function TableFilterString(props: TableFilterString): JSX.Element {
    ...
    */
    ...
    if (!smallScreenNameFiltering) {
        return (
            <form
                className={twMerge('flex flex-row flex-wrap justify-between items-baseline ', 'pr-2', className ?? '')}
                role="search"
            ...
            >
                ...
                <Filter column={column} placeholder={placeholder ?? `Filter ${name}`} className="hidden sm:block" />
