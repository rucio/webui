...
export const ListDID = (props: ListDIDPageProps) => {
    ...
    return (
        <div className={twMerge('flex flex-col space-y-2 w-full grow')}>
            <Heading title="List DIDs">
                <form className={twMerge('flex flex-col sm:flex-row sm:space-x-2 sm:items-end w-full ')} aria-label="DID Search">
                    <label className={twMerge('w-fit', 'text-text-1000 dark:text-text-0 ')} htmlFor="did-search-pattern">
                        DID Search Pattern
                    </label>
                    <div className="grow">
                        <TextInput
                    ...
                    <div className="w-full mt-2 sm:mt-0 sm:w-24 sm:grow-0">
                        {props.streamingHook.status === StreamingStatus.STOPPED ? (
                            <Button type="button" label="Search" onClick={onSearch} id="did-button-search" />
                        ) : (
                            <Button
                                type="button"
                                label="Stop"
                ...
                <div className={twMerge('w-full flex flex-col p-2 rounded', 'bg-neutral-200 dark:bg-neutral-900')}>
                    <form
                        className={twMerge('flex flex-col md:flex-row md:space-x-4 justify-space-between', 'text-text-800 dark:text-text-0')}
                        id="query-for-didtype-form"
                        aria-label="Select DID Types to Query"
                    >
                        <label className={twMerge('mr-2 text-text-1000 dark:text-text-0')} htmlFor="query-for-didtype-form">
                            Query for DID Types:
                        </label>
                        <Checkbox
                            label="Container"
            ...
            <Body className={twMerge('grid grid-rows-2 gap-y-2 lg:grid-rows-1 lg:grid-cols-3 lg:gap-y-0 lg:gap-x-2', 'grow')}>
                <div className={twMerge('bg-neutral-0 dark:bg-neutral-900', 'min-w-0', 'lg:col-span-2', 'flex flex-col h-[800px]')}>
                    <ListDIDTable streamingHook={props.streamingHook} tableRef={tableRef} onSelectionChanged={onSelectionChanged} />
                </div>
                <div className={twMerge('bg-neutral-100 dark:bg-neutral-900', 'rounded-md p-2', 'flex flex-col space-y-2')}>
                    <DIDMetaView data={meta} show={selected !== null} />
                    <div
                        className={twMerge('text-text-800 dark:text-text-100', selected === null ? 'block' : 'hidden')}
                        aria-label="Notice: No DID selected"
                    >
                        <i className="dark:text-text-200 text-text-1000">No DID selected</i>
                    </div>
                    <div className={twMerge(selected !== null ? '' : 'hidden')} aria-label="Go To DID Page">
                        <Link
                            className={twMerge(
