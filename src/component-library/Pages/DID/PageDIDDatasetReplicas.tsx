...
export const PageDIDDatasetReplicas = (props: { comdom: UseComDOM<DIDDatasetReplicasViewModel> }) => {
    const columnHelper = createColumnHelper<DIDDatasetReplicasViewModel>();
    const tablecolumns: any[] = [
        columnHelper.accessor('rse', {
            id: 'rse',
            header: info => {
                return <TableFilterString column={info.column} name="RSE" />;
            },
            cell: info => {
                return (
                    <span className="flex flex-row justify-between md:justify-start md:space-x-1 items-center pl-1">
                        <RSETag blocked={info.row.original.rseblocked}>
                            <a
                                href={`/rse/page/${info.row.id}`}
                                target="_blank"
                                rel="noreferrer"
                                className={twMerge('dark:text-text-0 text-text-1000', 'hover:underline')}
                            >
                                {info.getValue()}
                            </a>
                        </RSETag>
        ...
        columnHelper.accessor('available_files', {
            id: 'available_files',
            header: info => {
                return <TableSortUpDown name="Available Files" column={info.column} stack />;
            },
            cell: info => {
                return (
                    <span className={twMerge('flex flex-row justify-end pr-2', 'font-mono text-right dark:text-text-0 text-text-1000')}>
        ...
        columnHelper.accessor('available_bytes', {
            id: 'available_bytes',
            header: info => {
                return <TableSortUpDown name="Available Bytes" column={info.column} stack />;
            },
            cell: info => {
                return (
                    <span className={twMerge('flex flex-row justify-end pr-2', 'font-mono text-right dark:text-text-0 text-text-1000')}>
        ...
        columnHelper.accessor('creation_date', {
            id: 'creation_date',
            header: info => {
                return <TableSortUpDown name="Creation Date" column={info.column} stack />;
            },
            cell: info => {
                return (
                    <span className={twMerge('flex flex-row justify-end pr-2', 'font-mono text-right dark:text-text-0 text-text-1000')}>
        ...
        columnHelper.accessor('last_accessed', {
            id: 'last_accessed',
            header: info => {
                return <TableSortUpDown name="Last Accessed" column={info.column} stack />;
            },
            cell: info => {
                return (
                    <span className={twMerge('flex flex-row justify-end pr-2', 'font-mono text-right dark:text-text-0 text-text-1000')}>
