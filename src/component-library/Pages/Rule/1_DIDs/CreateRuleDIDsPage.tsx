...
import { Tabs } from '@/component-library/Misc/Tabs'
...
export const CreateRuleDIDsPage = (props: {
    ...
    useEffect(() => {
        ...
        if (
            PageDIDsState.searchDIDSelection.length === 0 &&
            PageDIDsState.typedDIDs.length === 0
        ) {
    ...
    const pageDIDsSearch = async (
        event: any,
        explicitDIDSearchExpression?: string,
    ) => {
        let DIDSearchString = explicitDIDSearchExpression
            ? explicitDIDSearchExpression
            : PageDIDsState.selectDIDDataPattern
    ...
    const pageDIDsnextFunction = (event: any) => {
        ...
        // execute query
        const typedDIDValidationResponse =
            props.CreateRulePageProps.didValidation(TypedDIDValidationQuery)
