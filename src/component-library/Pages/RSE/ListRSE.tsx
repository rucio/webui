...
export const ListRSE = (props: { comdom: UseComDOM<RSEViewModel>; setRSEQuery: (rseExpression: string) => void }) => {
    ...
    const tablecolumns = [
        columnHelper.accessor('name', {
            id: 'name',
            header: info => {
                return <TableFilterString column={info.column} name="Name" placeholder="Filter by RSE Name" className="ml-1" />;
            },
            cell: info => {
                    ...
                    <TableFilterDiscrete<RSEType>
                        ...
                        renderFunc={key =>
                            key === undefined ? (
                                ...
                                <RSETypeTag rsetype={key} forcesmall />
                            )
                        }
                        column={info.column}
                        stack
        ...
        columnHelper.accessor('volatile', {
            ...
            header: info => {
                return <TableFilterBoolean name="Volatile" column={info.column} stack />;
            },
            meta: {
                style: 'w-36',
            },
        ...
        columnHelper.accessor('deterministic', {
            ...
            cell: info => <BoolTag val={info.getValue()} />,
            header: info => {
                return <TableFilterBoolean name="Deterministic" column={info.column} stack />;
            },
            meta: {
                style: 'w-36',
            },
        ...
        columnHelper.accessor('staging_area', {
            ...
            cell: info => <BoolTag val={info.getValue()} />,
            header: info => {
                return <TableFilterBoolean name="Staging" column={info.column} stack />;
            },
            meta: {
                style: 'w-36',
            },
        ...
        <div className={twMerge('flex flex-col space-y-2 w-full')}>
            <Heading title="List RSEs">
                <form className={twMerge('flex flex-col sm:flex-row sm:space-x-2 sm:items-end w-full')} aria-label="RSE Search">
                    ...
                    <TextInput
                        onBlur={(event: any) => {
                            setInputAsQuery(event.target.value);
                        }}
                        onEnterkey={e => {
                            e.preventDefault();
