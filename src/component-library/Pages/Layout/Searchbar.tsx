        ...
        >
            ...
            >
                {props.searchLocations.map((location, index) => {
                    ...
                    >
                        <span>{location.parameter} <i>{searchQuery}</i> in {location.name}</span>
                    </LocationLink>
                })}
            </nav>
        </div>
...
export const Searchbar = () => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        } else if (query.includes(':')) { // Certainly a DID pattern
            setSearchLocations([didLocation])
        } else {
            if (/[=|&\\]/.test(query)) { // Certainly an RSE expression
                setSearchLocations([rseLocation])
            } else if (query.length === 32 && /^[a-zA-Z0-9]+$/.test(query)) { // Certainly a Rule UUID
                setSearchLocations([ruleLocation, rseLocation, didLocation])
            } else { // Anything but a UUID
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'ArrowDown') {
            setHighlightedIndex((prevIndex) =>
                prevIndex < searchLocations.length - 1 ? prevIndex + 1 : prevIndex
            )
