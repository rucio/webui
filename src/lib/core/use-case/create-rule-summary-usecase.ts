...
export default class CreateRuleSummayUseCase
    extends BaseUseCase<AuthenticatedRequestModel<CreateRuleSummaryRequest>, CreateRuleSummaryResponse, CreateRuleSummaryError>
    implements CreateRuleSummaryInputPort
{
    ...
    async execute(requestModel: AuthenticatedRequestModel<CreateRuleSummaryRequest>): Promise<void> {
        ...
        if (requestModel.withSamples) {
            ...
            requestModel.selectedDIDs.forEach(async (did: DID) => {
                const didDTO: DIDExtendedDTO = await this.didGateway.getDID(requestModel.rucioAuthToken, did.scope, did.name, DIDType.FILE);
                if (didDTO.status === 'success') {
                    const didSummaryRow: TDIDSummaryRow = createDIDSummaryRow(didDTO, requestModel.copies);
                    didSummaryTable.push(didSummaryRow);
                } else {
