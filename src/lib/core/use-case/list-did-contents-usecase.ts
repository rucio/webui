...
@injectable()
export default class ListDIDContentsUseCase
    extends BaseSingleEndpointStreamingUseCase<
        AuthenticatedRequestModel<ListDIDContentsRequest>,
        ...
        DIDViewModel
    >
    implements ListDIDContentsInputPort
{
    constructor(protected readonly presenter: ListDIDContentsOutputPort, private readonly gateway: DIDGatewayOutputPort) {
        super(presenter);
    }
    ...
    async intializeRequest(
        request: AuthenticatedRequestModel<AuthenticatedRequestModel<ListDIDContentsRequest>>,
    ): Promise<ListDIDContentsError | undefined> {
        return undefined;
    }
    ...
    processStreamedData(dto: DIDDTO): { data: ListDIDContentsResponse | ListDIDContentsError; status: 'success' | 'error' } {
        if (dto.status === 'error') {
            const errorModel: ListDIDContentsError = {
                status: 'error',
