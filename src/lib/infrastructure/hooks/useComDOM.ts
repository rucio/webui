...
import ComDOMWrapper, { BatchResponse, ComDOMStatus, IComDOMWrapper } from '@/lib/infrastructure/web-worker/comdom-wrapper';
...
export default function useComDOM<TData extends BaseViewModel>(
    ...
    const [comDOMStatus, setComDOMStatus] = useState<ComDOMStatus>(ComDOMStatus.UNKNOWN);
    ...
    const [status, setStatus] = useState<UseComDOMStatus>(UseComDOMStatus.STOPPED);
    ...
    const queryFn = async () => {
        if (comDOMStatus == ComDOMStatus.STOPPED || comDOMStatus == ComDOMStatus.ERROR) {
            _log('ComDOM Web Worker is not running. The query will not fetch any data', 'ComDOM Status:', comDOMStatus);
            return Promise.reject('ComDOM Web Worker has not been created or it is not running. The query will not fetch any data');
        }
        try {
            setStatus(UseComDOMStatus.RUNNING);
            let batchResponse: BatchResponse<TData> | null = await comDOMWrapper.next();
