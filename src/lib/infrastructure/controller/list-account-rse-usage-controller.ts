...
@injectable()
class ListAccountRSEUsageController extends BaseController<ListAccountRSEUsageControllerParameters, AuthenticatedRequestModel<ListAccountRSEUsageRequest>> {
    constructor(
        @inject(USECASE_FACTORY.LIST_ACCOUNT_RSE_USAGE) listAccountRSEUsageUseCaseFactory: (response: NextApiResponse) => ListAccountRSEUsageInputPort,
    ) {
