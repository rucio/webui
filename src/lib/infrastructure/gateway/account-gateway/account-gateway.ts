...
export default class RucioAccountGateway implements AccountGatewayOutputPort {
    ...
    async listAccountAttributes(account: string, rucioAuthToken: string): Promise<AccountAttributesDTO> {
        ...
        try {
            ...
            return Promise.reject(this.generateErrorResponse(AccountAttributeErrorTypesDTO.RUCIO_HOST_NOT_CONFIGURED, account, error.message));
        }
        ...
        if (!response) {
            return Promise.reject(
                this.generateErrorResponse(
                    AccountAttributeErrorTypesDTO.RUCIO_SERVER_DID_NOT_RETURN_A_RESPONSE,
                    account,
                    'Rucio Server did not return a response',
                ),
            );
        }
        ...
        if (response.status === 401) {
            return Promise.reject(
                this.generateErrorResponse(
                    AccountAttributeErrorTypesDTO.RUCIO_AUTH_TOKEN_IS_INVALID_OR_EXPIRED,
                    account,
                    'Rucio Auth Token is invalid or expired',
                ),
            );
        } else if (response.status !== 200) {
            return Promise.reject(this.generateErrorResponse(AccountAttributeErrorTypesDTO.UNKNOWN_ERROR, account, 'Unknown Error'));
        }
