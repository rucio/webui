...
> {
    constructor(appContainer: Container) {
        ...
        super('GetSiteHeader', GetSiteHeaderController, GetSiteHeaderUseCase, [envConfigGateway], GetSiteHeaderPresenter, false, symbols);
    }
    ...
    load(appContainer: Container) {
        ...
        appContainer
            .bind<interfaces.Factory<GetSiteHeaderInputPort>>(USECASE_FACTORY.GET_SITE_HEADER)
            .toFactory<GetSiteHeaderUseCase, [NextApiResponse]>((context: interfaces.Context) => (response: NextApiResponse) => {
                const envConfigGateway: EnvConfigGatewayOutputPort = appContainer.get(GATEWAYS.ENV_CONFIG);
